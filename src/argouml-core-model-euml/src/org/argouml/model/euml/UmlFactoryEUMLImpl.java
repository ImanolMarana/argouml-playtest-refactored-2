...
//        {Message.class, ClassifierRole.class },
    ...
    public Object buildConnection(Object elementType, Object fromElement,
        ...
        Object connection = buildSpecificConnection(elementType, fromElement,
    ...
    private Object buildSpecificConnection(Object elementType, Object fromElement,
            Object fromStyle, Object toElement, Object toStyle,
            Object unidirectional, Object namespace) {
        
        if (elementType == metaTypes.getAssociationRole()) {
            return Model.getCollaborationsFactory().buildAssociationRole(
                    fromElement, toElement);
        } else if (elementType == metaTypes.getAssociation()) {
            // Note for UML2 the aggregation ends are swapped
            return modelImpl.getCoreFactory().buildAssociation(
            ...
            return modelImpl.getCoreFactory().buildAssociationClass(
                    fromElement, toElement);
        } else if (elementType == metaTypes.getGeneralization()) {
            return modelImpl.getCoreFactory().buildGeneralization(
                    fromElement, toElement);
        } else if (elementType == metaTypes.getPackageImport()) {
            return modelImpl.getCoreFactory().buildPackageImport(
                    fromElement, toElement);
        } else if (elementType == metaTypes.getUsage()) {
            return modelImpl.getCoreFactory().buildUsage(
                    fromElement, toElement);
        } else if (elementType == metaTypes.getComponentRealization()) {
            return modelImpl.getCoreFactory().buildComponentRealization(
                    fromElement, toElement);
        } else if (elementType == metaTypes.getDependency()) {
            return modelImpl.getCoreFactory().buildDependency(
                    fromElement, toElement);
        } else if (elementType == metaTypes.getAbstraction()) {
            return modelImpl.getCoreFactory().buildRealization(
                    fromElement, toElement, namespace);
        } else if (elementType == metaTypes.getLink()) {
            return modelImpl.getCommonBehaviorFactory().buildLink(
                    fromElement, toElement);
            ...
            return modelImpl.getUseCasesFactory().buildExtend(
                    toElement, fromElement);
        } else if (elementType == metaTypes.getInclude()) {
            return modelImpl.getUseCasesFactory().buildInclude(
                    fromElement, toElement);
        } else if (elementType == metaTypes.getControlFlow()) {
            return buildControlFlow(fromElement, toElement);
        } else if (elementType == metaTypes.getObjectFlow()) {
            return buildObjectFlow(fromElement, toElement);
        } else if (elementType == metaTypes.getTransition()) {
            return modelImpl.getStateMachinesFactory().buildTransition(
                    fromElement, toElement);
        } else if (elementType == metaTypes.getAssociationRole()) {
            return modelImpl.getCollaborationsFactory().buildAssociationRole(
    ...
    private Object buildAssociationEnd(Object fromElement, Object toElement) {
        if (fromElement instanceof Association) {
            return modelImpl.getCoreFactory().buildAssociationEnd(
                    toElement, fromElement);
    ...
    private Object buildControlFlow(Object fromElement, Object toElement) {
    ...
    private Object buildObjectFlow(Object fromElement, Object toElement) {
