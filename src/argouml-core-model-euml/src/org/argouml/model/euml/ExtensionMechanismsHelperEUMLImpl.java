...
class ExtensionMechanismsHelperEUMLImpl implements ExtensionMechanismsHelper {
    ...
    public void removeBaseClass(Object handle, Object baseClass) {
        ...
        removeBaseClass(metaclass, (Stereotype) handle, profile);
    }

    private void removeBaseClass(org.eclipse.uml2.uml.Class metaclass,
            Stereotype stereotype, Profile profile) {
        for (Extension ext : profile.getOwnedExtensions(false)) {
            if (ext.getMetaclass() != metaclass
                    || !ext.getEndTypes().contains(stereotype)) {
                continue;
            }
            removeExtension(stereotype, ext);
            break;
        }
    }

    private void removeExtension(Stereotype stereotype, Extension ext) {
