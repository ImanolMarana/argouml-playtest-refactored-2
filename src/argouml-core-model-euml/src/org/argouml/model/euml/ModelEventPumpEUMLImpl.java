...
class ModelEventPumpEUMLImpl extends AbstractModelEventPump {
    ...
    void fireEvent(
        ...
        if (eventType == Notification.SET) {
            events.add(createAttributeChangeEvent(notifier, featureName, oldValue, newValue));
        } else if (eventType == Notification.ADD
                || eventType == Notification.REMOVE) {
            handleAddRemoveEvent(notifier, oldValue, newValue, eventType, featureName, oppositeRef, events);
        }

        fireEventForListeners(events);
    }
    
    private EventAndListeners createAttributeChangeEvent(Object notifier, String featureName, Object oldValue, Object newValue) {
        String propName = mapPropertyName(featureName);
        return new EventAndListeners(new AttributeChangeEvent(
                notifier, propName,
                oldValue, newValue,
                null), getListeners(
                notifier, propName));
    ...
    private void handleAddRemoveEvent(Object notifier, Object oldValue, Object newValue, int eventType, String featureName, EReference oppositeRef, List<EventAndListeners> events) {
        String propName = mapPropertyName(featureName);
        if (eventType == Notification.ADD) {
            events.add(createAddAssociationEvent(notifier, propName, newValue));
            events.add(createAttributeChangeEvent(notifier, propName, null, newValue));
        } else {
            handleRemoveEvent(notifier, oldValue, propName, events);
        }

        if (oppositeRef != null) {
            handleOppositeReference(notifier, oldValue, newValue, eventType, oppositeRef, events);
        }
    }

    private void handleOppositeReference(Object notifier, Object oldValue, Object newValue, int eventType, EReference oppositeRef, List<EventAndListeners> events) {
        String propName = mapPropertyName(oppositeRef.getName());
        if (eventType == Notification.ADD) {
            events.add(createAddAssociationEvent(newValue, propName, notifier));
            events.add(createAttributeChangeEvent(newValue, propName, null, notifier));
        } else { // i.e. eventType == Notification.REMOVE
            events.add(createAddAssociationEvent(oldValue, propName, notifier));
    ...
    private EventAndListeners createAddAssociationEvent(Object notifier, String propName, Object newValue) {
        return new EventAndListeners(new AddAssociationEvent(
    ...
    private void handleRemoveEvent(Object notifier, Object oldValue, String propName, List<EventAndListeners> events) {
    ...
    private void fireEventForListeners(List<EventAndListeners> events) {
        for (EventAndListeners e : events) {
            if (e.listeners != null) {
                for (EventListener l : e.listeners) {
                    if (l instanceof AssociationChangeListener) {
                        fireEventForAssociationChangeListener(e, l);
    ...
    private void fireEventForAssociationChangeListener(EventAndListeners e, EventListener l) {
