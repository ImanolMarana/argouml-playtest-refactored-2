...
//	    }
    ...
    private JComboBox uiSelect() {
        ...
        int iSel = findSelectedIndex(typeInstanceValue);
      ...
      private int findSelectedIndex(Object[] typeInstanceValue) {
        ...
        if (model != null) {
            ...
            if (expression != null) {
                ...
                for (int iInterf = 0; iInterf < interfaces.length && iSel == -1; iInterf++) {
                    iSel = findMatchingTypeIndex(typeInstanceValue, interfaces[iInterf]);
                }
            }
        }
        return iSel;
      ...
      private int findMatchingTypeIndex(Object[] typeInstanceValue, Class<?> interfaceType) {
        for (int iVS = 0; iVS < typeInstanceValue.length; iVS++) {
            if (interfaceType.getSimpleName().equals(typeInstanceValue[iVS])) {
                return iVS;
            }
        }
        return -1;
