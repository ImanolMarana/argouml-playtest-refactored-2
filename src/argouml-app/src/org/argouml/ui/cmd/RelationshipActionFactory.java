...
public class RelationshipActionFactory implements ContextActionFactory {
    ...
    private ActionList getAssociations(final Object element, ArgoDiagram diagram) {
        ...
        if (Model.getFacade().isAClass(element)) {
          ...
          for (Object associationEnd : associationEnds) {
            addAssociationAction(element, diagram, al, associationEnd);
          }
        }
        return al;
      }

      private void addAssociationAction(final Object element, ArgoDiagram diagram, ActionList al,
        ...
        if (diagram.presentationFor(association) == null) {
          Collection connections = Model.getFacade().getConnections(association);
          if (connections.size() == 2) {
            addAssociationActionForConnection(element, diagram, al, associationEnd, association,
                connections);
          }
        }
      }

      private void addAssociationActionForConnection(final Object element, ArgoDiagram diagram,
          ActionList al, Object associationEnd, Object association, Collection connections) {
        for (Object connection : connections) {
          if (connection != associationEnd) {
            final String direction = Model.getFacade().isNavigable(associationEnd) ? "from" : "to";
            Object oppositeClass = Model.getFacade().getClassifier(connection);
            al.add(new AddAssociationAction(
                "Add association " + direction + " " + Model.getFacade().getName(oppositeClass),
                diagram, element, association, oppositeClass));
          }
        }
      }
